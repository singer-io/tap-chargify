...
class TestIncrementalSyncHelpers(unittest.TestCase):
    ...
    def test_is_after_bookmark(self):
        stream = Transactions(MagicMock())
        state = {"bookmarks": {"transactions": {"created_at": "2025-01-01T00:00:00Z"}}}

        self.assertTrue(stream.is_after_bookmark(state, "2025-01-02T00:00:00Z"))
        self.assertFalse(stream.is_after_bookmark(state, "2024-12-30T00:00:00Z"))
...
class TestInvoicesBookmark(unittest.TestCase):
    """Invoices.is_after_bookmark must handle plain date strings (YYYY-MM-DD)
    and epoch timestamps without raising a TypeError on non-UTC hosts.
    """
    ...
    def test_date_only_value_newer_than_bookmark(self):
        ...
        # 2025-04-20 > 2025-04-01 → value is after bookmark
        self.assertTrue(stream.is_after_bookmark(state, "2025-04-20"))

    def test_date_only_value_older_than_bookmark(self):
        stream = Invoices(MagicMock())
        state = {"bookmarks": {"invoices": {"due_date": "2025-04-20"}}}
        self.assertFalse(stream.is_after_bookmark(state, "2025-03-01"))

    def test_date_only_equal_to_bookmark(self):
        stream = Invoices(MagicMock())
        state = {"bookmarks": {"invoices": {"due_date": "2025-04-20"}}}
        self.assertFalse(stream.is_after_bookmark(state, "2025-04-20"))

    def test_fallback_to_start_date_when_no_state(self):
        stream = Invoices(MagicMock())
        state = {}
        # start_date is 2025-01-01T00:00:00Z; normalised to 2025-01-01
        # 2025-06-01 > 2025-01-01 → value is after bookmark
        self.assertTrue(stream.is_after_bookmark(state, "2025-06-01"))
    ...
    def test_datetime_value_compared_as_date(self):
        ...
        self.assertTrue(stream.is_after_bookmark(state, "2025-04-21T00:00:00Z"))

    def test_epoch_timestamp_value(self):
        """An epoch timestamp (int) should be handled via datetime lib."""
        stream = Invoices(MagicMock())
        state = {"bookmarks": {"invoices": {"due_date": "2025-04-20"}}}
        # 1745280000 == 2025-04-22 UTC
        self.assertTrue(stream.is_after_bookmark(state, 1745280000))
    ...
    def test_does_not_raise_on_tz_naive_date(self):
        ...
        try:
            result = stream.is_after_bookmark(state, "2025-06-01")
        except TypeError as exc:
            self.fail(f"is_after_bookmark raised TypeError for a plain date: {exc}")
